		...
		Function: model.ToolFunctionDefinition{
			...
			Parameters: map[string]any{
				...
				"properties": map[string]any{
					...
					"pattern":   map[string]any{"type": "string", "description": "File pattern, such as **/*.go"},
					"limit":     map[string]any{"type": "integer", "description": "Maximum number of paths to return (default: 1000)"},
				},
...
func (g *glob) Run(ctx context.Context, input model.ToolCallFunctionArguments) (string, error) {
	...
	err = filepath.WalkDir(args.Directory, func(path string, entry fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		select {
		...
		if parts[0] == ".git" || matcher.Match(parts, entry.IsDir()) {
			if entry.IsDir() {
